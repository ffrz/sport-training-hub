...
const goToUrl = (url, newFilter) => {
  ...
  // localStorage.setItem('bizia-sth.service-orders.filter', JSON.stringify(mergedFilter));
  router.visit(url, {
    // data: mergedFilter
  });
}
        ...
        <div class="row q-col-gutter-sm ">
          ...
          <div class="col-sm-4 col-xs-12">
            <q-item :style="`background-color: #008cff`" class="q-pa-none" clickable
              @click="$inertia.visit(route('coach.index', { status: 'active' }))">
              ...
              <q-item-section class=" q-pa-md q-ml-none  text-white">
                <q-item-label class="text-white text-h6 text-weight-bolder">
                  {{ page.props.summary.active_coach_count }}
                </q-item-label>
                <q-item-label>Pelatih Aktif</q-item-label>
              </q-item-section>
            </q-item>
          </div>
          <div class="col-sm-4 col-xs-12">
            <q-item :style="`background-color: #008cff`" class="q-pa-none" clickable
              @click="$inertia.visit(route('student.index', { status: 'active' }))">
              <q-item-section side :style="`background-color: #007bff`" class="q-pa-lg q-mr-none text-white">
                <q-icon class="material-filled" name="groups_2" color="white" size="24px" />
              </q-item-section>
              <q-item-section class=" q-pa-md q-ml-none  text-white">
                <q-item-label class="text-white text-h6 text-weight-bolder">
                  {{ page.props.summary.active_student_count }}
                </q-item-label>
                <q-item-label>Siswa Aktif</q-item-label>
              </q-item-section>
            </q-item>
          </div>
          <div class="col-sm-4 col-xs-12">
            <q-item :style="`background-color: #008cff`" class="q-pa-none" clickable
              ...
              <q-item-section class=" q-pa-md q-ml-none  text-white">
                <q-item-label class="text-white text-h6 text-weight-bolder">
                  {{ page.props.summary.active_user_count }}
                </q-item-label>
